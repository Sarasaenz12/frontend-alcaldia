document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("crearFuncionarioForm").addEventListener("submit", async function (e) {
    ...
    const url = editandoId
      ? `https://backend-alcaldia-5.onrender.com/api/auth/users/${editandoId}/`
      : "https://backend-alcaldia-5.onrender.com/api/auth/users/create/";
  ...
  async function cargarFuncionarios() {
    const res = await fetch("https://backend-alcaldia-5.onrender.com/api/auth/users/", {
      headers: {
        Authorization: `Bearer ${token}`,
      }
    });
  ...
  window.eliminarFuncionario = async function (id) {
    ...
    const res = await fetch(`https://backend-alcaldia-5.onrender.com/api/auth/users/${id}/`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`
      }
